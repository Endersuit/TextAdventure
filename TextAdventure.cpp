...
int main()
{
    Creature* player = new Creature("Kalagan", 15, 4, 10, false, std::make_unique<IncendiaryBomb>());

    //puntatori smart unici -> gestione automatica della memoria
    //nemici Round One
    std::unique_ptr<Creature> endersuit = std::make_unique<Creature>("Endersuit", 14, 6, 5, true, std::make_unique<HealingPotion>());
    std::unique_ptr<Creature> enderman = std::make_unique<Creature>("Enderman", 10, 2, 2, true, std::make_unique<IncendiaryBomb>());
    std::unique_ptr<Creature> endermite = std::make_unique<Creature>("Endermite", 5, 1, 1, false, nullptr);
